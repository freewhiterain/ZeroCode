...
from zerocode.worktree.models import Worktree, WorktreeSession
from zerocode.worktree.session import load_worktree_session, save_worktree_session
from zerocode.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._ZeroCode_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._ZeroCode_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._ZeroCode_dir)
        if session is None:
            return None
